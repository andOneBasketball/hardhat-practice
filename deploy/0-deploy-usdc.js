...
module.exports = async ({ getNamedAccounts, deployments }) => {
    ...
    const usdt = await deploy("UsdcToken", {
        from: deployer,
        log: true,
        waitConfirmations: waitBlockConfirmations,
    });
    console.log("UsdcToken address " + usdt.address);
    const Contract = await hre.ethers.getContractFactory("UsdcToken");
    ...
    if (!developmentChains.includes(network.name) && process.env.EVM_SCAN_API_KEY) {
        ...
        await verify(usdt.address, [], "contracts/UsdcToken.sol:UsdcToken");
    }
};
